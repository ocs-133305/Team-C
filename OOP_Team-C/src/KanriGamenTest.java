...
public class KanriGamenTest extends JFrame{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					KanriGamenTest frame = new KanriGamenTest();
	...
	public KanriGamenTest() throws Exception{
					...
					try{
						...
						if(db.connect()){
							...
							if(rs.next()){
								...
								bclassComboBox.setSelectedIndex(rs.getInt("class_id"));
				...
				if(blengthCheck()){
					try{
						...
						if(db.connect()){
							...
							int bid = 0;                            // 図書番号 
							ResultSet rs = db.select("SELECT MAX(book_id) FROM book");
							...
							for(int i=0;i<bnum;i++){
								db.update("INSERT INTO book VALUES("+bid+",'" + btitle +"','"+bauthor+"',"+ class_id + ",'" + company + "'," + isbnval +")");
							}
							//　メッセージ表示
							if(bnum == 1){
								messageField.setText("図書管理番号：" + bid + "として登録しました");
							}else{
				...
				if(blengthCheck()){
					try{
						if(db.connect()){
							...
							//　SQL文構築（仮）
							sqlstr = "UPDATE book SET book_name = '"+btitle+"',author ='" + bauthor + "',class_id = " + class_id + ",company ='" + company + "',isbn =" + isbnval +"WHERE book_id="+bidval;
					...
					try{
						...
						try{
							if(db.connect()){
								...
								ResultSet rs = db.select("SELECT MAX(user_id) FROM user");
								if(rs.next()){
									uid = rs.getInt(1)+1;
								}
								
								//　SQLを使った処理（インサート）
								db.update("INSERT INTO user VALUES("+uid+",'" + uname +"','"+ uaddress +"',"+ upostval + "," + uphoneval + ",'" + umail +"')");
								
								//　メッセージ表示
								messageField.setText("会員番号：" + uid + "として登録しました");
					...
					try{
						...
						try{
							if(db.connect()){
								...
								//　SQL文構築
								sqlstr = "UPDATE user SET user_name = '"+ uname +"', address ='" + uaddress + "',post = " + upostval + ",phone =" + uphoneval + ",mail ='" + umail +"' WHERE user_id="+uidval;
